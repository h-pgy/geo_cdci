...
class LogradouroLineFetcher:
    ...
    def convert_segments_to_gdf(self, segmentos: list[dict[str, Any]], reprojetar_para_3857: bool = False) -> gpd.GeoDataFrame:
        if not segmentos:
            return gpd.GeoDataFrame()
            
        gdf = gpd.GeoDataFrame.from_features(segmentos, crs=self.base_crs)
        
        if reprojetar_para_3857:
            gdf = gdf.to_crs(epsg=3857)
            
        return gdf
    
    def __call__(self, codlog: str, reprojetar_para_3857: bool = False) -> gpd.GeoDataFrame:
        segmentos = self.get_line_segments_by_codlog(codlog)
        return self.convert_segments_to_gdf(segmentos, reprojetar_para_3857)
